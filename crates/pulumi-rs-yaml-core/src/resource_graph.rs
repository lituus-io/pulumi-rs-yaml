...
fn single_name_access<'src>(access: &'src PropertyAccess<'src>) -> Option<&'src str> {
...
fn resolve_variable_literal<'src>(
    name: &'src str,
    variables: &HashMap<&'src str, &'src Expr<'src>>,
    memo: &mut HashMap<&'src str, Option<Cow<'src, str>>>,
    visiting: &mut HashSet<&'src str>,
...
/// literals.
fn format_literal_number(n: f64) -> Option<String> {
    if !n.is_finite() {
        return None;
    }
...
fn collect_literal_properties<'src>(
    entry: &'src ResourceEntry<'src>,
    variables: &HashMap<&'src str, &'src Expr<'src>>,
